...
const getSerieInfo = async(name) => {
  ...
  return { poster: '', background: '', overview: '' }
}
...
const create = ({ db }) => async(req, res) => {
  ...
  const serie = await getSerieInfo(newSerie.name)

  const serieToInsert = {
    name: newSerie.name,
    status: newSerie.status,
    genre_id: newSerie.genre_id,
    comments: serie.overview,
    poster: serie.poster,
    background: serie.background
  }

  console.log(serieToInsert)
...
const update = ({ db }) => async(req, res) => {
  ...
  const images = await getSerieInfo(updatedSerie.name)
